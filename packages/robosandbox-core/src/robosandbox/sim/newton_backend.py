...
# ---------------------------------------------------------------------------
# Newton cloth backend — wraps ClothFrankaShowcase
# ---------------------------------------------------------------------------

class NewtonClothBackend:
    """Newton backend specialised for cloth + Franka demos (VBD solver).

    scene.load() is a no-op here: the scene geometry (USD shirt mesh,
    Franka URDF from Newton assets) is built entirely by ClothFrankaShowcase
    and does not depend on the task YAML's scene definition.

    The step() method drives one full showcase frame (10 physics substeps +
    Viser render). The server idle loop should call step() continuously to
    keep the cloth physics running.
    """
    ...
    def _ensure_runtime(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "Newton cloth backend requires `warp` installed in the active environment"
            ) from e
        self._wp = wp

    def load(self, scene: "Scene") -> None:  # noqa: F821 – forward ref
        self._ensure_runtime()
        self._wp.set_device(self._device)
        ...
        try:
            ...
            raise ImportError(
                "Newton cloth backend requires `newton` installed in the active environment"
            ) from e

        if self._viewer_kind == "viser":
            viewer = ViewerViser(port=self._port)
        else:
            viewer = ViewerNull(num_frames=1_000_000)

        from robosandbox.sim.cloth_showcase import ClothFrankaShowcase
        self._showcase = ClothFrankaShowcase(viewer=viewer, world_count=self._world_count)

    def step(
        self,
        target_joints: "np.ndarray | None" = None,  # noqa: F821
        gripper: "float | None" = None,  # noqa: F821
    ) -> None:
        assert self._showcase is not None, "call load() first"
        self._showcase.step()
        self._showcase.render()

    def observe(self) -> "Observation":  # noqa: F821
        from robosandbox.types import Observation, Pose
        return Observation(
            rgb=np.zeros((480, 640, 3), dtype=np.uint8),
            depth=None,
            robot_joints=np.zeros(9, dtype=np.float64),
            ee_pose=Pose(xyz=(0.0, 0.0, 0.0), quat_xyzw=(0.0, 0.0, 0.0, 1.0)),
            gripper_width=0.0,
            scene_objects={},
            timestamp=self._showcase.sim_time if self._showcase else 0.0,
            camera_intrinsics=None,
            camera_extrinsics=None,
        )
    ...
    def close(self) -> None:
        if self._showcase is not None:
            try:
                self._showcase.viewer.close()
            except Exception:
                pass
            self._showcase = None

    def get_object_pose(self, object_id: str) -> None:
        return None

    def set_object_pose(self, object_id: str, pose: Any) -> None:
        raise NotImplementedError("Newton cloth backend does not support set_object_pose")

    @property
    def n_dof(self) -> int:
        return 9  # 7 arm + 2 fingers
    ...
    @property
    def continuous_step(self) -> bool:
        """True: server idle loop should call step() continuously."""
        return True
