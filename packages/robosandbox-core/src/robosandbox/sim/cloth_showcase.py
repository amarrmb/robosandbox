"""ClothFrankaShowcase — Newton cloth+robot physics scene.

Packaged here so both the standalone example script and the
newton_cloth sim backend can share the same implementation.
"""
...
# ---------------------------------------------------------------------------
# Warp kernels (module-level so they are compiled once)
# ---------------------------------------------------------------------------
...
try:
    ...
    @wp.kernel
    def _scale_positions(src: wp.array[wp.vec3], scale: float, dst: wp.array[wp.vec3]):
        i = wp.tid()
        dst[i] = src[i] * scale

    @wp.kernel
    def _scale_body_transforms(
        src: wp.array[wp.transform], scale: float, dst: wp.array[wp.transform]
    ):
        i = wp.tid()
...
class ClothFrankaShowcase:
    """Newton + Warp cloth + Franka simulation, 1 or N parallel worlds."""

    def __init__(self, viewer: Any, world_count: int = 1) -> None:
        if not _WARP_AVAILABLE:
            raise ImportError("ClothFrankaShowcase requires warp")
        ...
        try:
            ...
            raise ImportError("ClothFrankaShowcase requires newton") from exc

        try:
            from pxr import Usd
        except ImportError as exc:
            raise ImportError("ClothFrankaShowcase requires pxr (USD)") from exc

        self._newton = newton
        self._eval_fk = eval_fk
        self._Usd = Usd
        ...
        self.table_viz_color = wp.array(
            [wp.vec3(0.82, 0.72, 0.42)] * self.world_count, dtype=wp.vec3
        )
        ...
        self.viz_shape_transform = wp.array(
            xform_np, dtype=wp.transform, device=self.model.device
        )
        ...
        self._setup_control()
        self._eval_fk(self.model, self.model.joint_q, self.model.joint_qd, self.state_0)

    # -------------------------------------------------------------------------

    def _build_world(self) -> Any:
        import newton
        import newton.examples
        import newton.usd
        ...
        usd_stage = self._Usd.Stage.Open(newton.examples.get_asset("unisex_shirt.usd"))
    ...
    def _demo_world_offsets(self, world_count: int) -> list[tuple[float, float, float]]:
        ...
        spacing_x, spacing_y = 160.0, 240.0
    ...
    def _create_robot(self, builder: Any) -> None:
        import newton.utils
        ...
        self.initial_pose = np.array(
            builder.joint_q[: builder.joint_coord_count], dtype=np.float32
        )
    ...
    def _compute_body_jacobian(
        self, joint_q_np: np.ndarray, joint_qd_np: np.ndarray
    ) -> np.ndarray:
        ...
        tape = wp.Tape()
        with tape:
            self._eval_fk(self.model_single, joint_q, joint_qd, self.jac_state)
            wp.launch(
                self.compute_body_out_kernel,
                dim=1,
                inputs=[
                    self.jac_state.body_q,
                    self.jac_state.body_qd,
                    self.model_single.body_com,
                ],
        ...
        for i in range(6):
            ...
            wp.copy(
                self.J_flat[i * self.dof_qd_per_world : (i + 1) * self.dof_qd_per_world],
                joint_qd.grad,
            )
            tape.zero()
        return self.J_flat.numpy().reshape(6, self.dof_qd_per_world)

    def _target_transforms(self, target_row: np.ndarray) -> Any:
    ...
    def _solve_damped_least_squares(
        self, J: np.ndarray, delta_target: np.ndarray
    ) -> np.ndarray:
        J = np.nan_to_num(J, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float64, copy=False)
        delta_target = np.nan_to_num(
            delta_target, nan=0.0, posinf=0.0, neginf=0.0
